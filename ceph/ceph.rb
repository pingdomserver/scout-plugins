class CephPlugin < Scout::Plugin
  ...
  class CephStatus
    ...
    def parse
      ...
      @status[:num_osds] = search_in_output(@lines, /(\d*)\sosds:/)[1].to_i
      @status[:osds_up] = search_in_output(@lines, /(\d*)\sup/)[1].to_i
      @status[:osds_in] = search_in_output(@lines, /(\d*)\sin/)[1].to_i
      @status[:data_size] = clean_value(search_in_output(@lines, /\s*(\d*\s[A-Z]{2})\sdata/)[1])
      @status[:used] = clean_value(search_in_output(@lines, /\s*(\d*\s[A-Z]{2})\sused/)[1])
      @status[:available] = clean_value(search_in_output(@lines, /\s*(\d*\s[A-Z]{2})\s\//)[1])
      @status[:cluster_total_size] = clean_value(search_in_output(@lines, /\s*(\d*\s[A-Z]{2})\savail/)[1])
    ...
    def search_in_output(lines, pattern) # shall I keep the position
      position = 0
      value = lines[position].match(pattern)

      while !value && position < lines.length
        position += 1
        value = lines[position].match(pattern)
      end
