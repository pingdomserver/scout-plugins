...
class MysqlQueryStatistics < Scout::Plugin
  ...
  tuner_days:
    name: MySQLTuner Summary Frequency
  ...
  def build_report
    ...
    if last_run == nil || (last_run.is_a?(Time) && Time.now - days*86400 >= last_run)

      # give an error message if any of the needed command lines can't be found
      checks = [`which perl`, `which mysql`, `which mysqladmin`]
      if checks.any?{|c|c.strip ==''}
        error_text=<<-END_ERROR_TEXT
        MySQLTuner needs perl, mysql, and mysqladmin to run. One or more of these are not available:
        `which perl` output:#{checks[0]}
        `which mysql` output:#{checks[1]}
        `which mysqladmin` output:#{checks[2]}
        ----
        Your $PATH in this environment is:
        #{ENV['PATH'].gsub(":","\n")}

        To solve this issue:
          1) ensure that perl, mysql, and mysqladmin are installed;
          2) ensure that all three executables are available in your PATH.

        Check your PATH carefully. PATH is often different when run from a cron job.
        END_ERROR_TEXT
        error("Can not run MySQLTuner", error_text)
      else
        ...
        #record tuner results in a summary
        summary( :command => "mysqltuner.pl", :output => "<pre style='font-size:smaller'>"+analysis+"</pre>" )
        remember(:tuner_last_run, Time.now)
      end
    else
...
sub mysql_setup {
  ...
  if ($opt{user} ne 0 ) {
    my $mysqlpassstring = ($opt{pass} ne 0)? "-p'$opt{pass}'" : "";
    $mysqllogin = "-u $opt{user} $mysqlpassstring".$remotestring;
